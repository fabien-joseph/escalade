...
public interface SiteRepository extends CrudRepository<Site, Long> {
    ...
    @Query("SELECT s FROM Site s WHERE lower(s.nom) like %:nom% AND (s.departement = :departement OR '' = :departement) AND " +
            "s.hauteurMin <= :hauteurMin AND s.hauteurMax <= :hauteurMax AND s.cotationMin >= :cotationMin AND s.cotationMax <= :cotationMax")
    List<Site> findSitesAdvancedTest(@Param("nom") String nom, @Param("departement") String departement,
                                     @Param("hauteurMin") Integer hauteurMin, @Param("hauteurMax") Integer hauteurMax,
                                     @Param("cotationMin") Integer cotationMin, @Param("cotationMax") Integer cotationMax);
